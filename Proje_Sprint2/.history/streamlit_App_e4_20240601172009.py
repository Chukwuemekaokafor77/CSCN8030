# Streamlit App Script# Streamlit App Script# Streamlit App Script
    ...
    for option, checked in selected_options.items():
        if checked:
            ...
            if option == "Stock Actions":
                ...
                # Predict prices
                predicted_prices = model.predict(selected_stock)
...
def main():
    ...
    for selected_stock in selected_stocks:
        ...
        # Load the trained model
        best_model = joblib.load(f"{selected_stock}_model.pkl")  # Adjust the path accordingly
        
        # Call the function to plot the prediction graph here
        # Replace `results` with the actual results from your prediction
        # For example:
        # results = {"AAPL": {"y_test": ..., "y_pred": ...}, "GOOGL": {"y_test": ..., "y_pred": ...}}
        # plot_prediction_graph(results)

        if selected_options["Predicted Prices"]:
            # Predict prices
            predicted_prices = best_model.predict(selected_stock)
            st.write(predicted_prices)
