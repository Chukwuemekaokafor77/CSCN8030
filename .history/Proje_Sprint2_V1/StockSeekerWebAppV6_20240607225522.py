...
def calculate_stochastic_oscillator(data, period=14):
    ...
    stoch_results = ta.stoch(data['High'], data['Low'], data['Close'], window=period)
    print(stoch_results.columns)
    # Extract %K and %D from the results
    percent_K = stoch_results['STOCHk_14']
    percent_D = stoch_results['STOCHd_14']
