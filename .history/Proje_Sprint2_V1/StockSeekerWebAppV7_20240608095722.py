...
def display_predicted_prices(selected_stock, start_date, end_date):
    ...
    fig.add_trace(go.Scatter(x=valid.index, y=valid['Close'], mode='lines', name='Actual Prices'))
    fig.add_trace(go.Scatter(x=valid.index, y=valid['Predictions'], mode='lines', name='Predicted Prices'))
    fig.update_layout(title=f'{selected_stock} Price Prediction',
                      xaxis_title='Date',
                      yaxis_title='Price')
    st.plotly_chart(fig)

# Function to display Stochastic Oscillator and Moving Averages
