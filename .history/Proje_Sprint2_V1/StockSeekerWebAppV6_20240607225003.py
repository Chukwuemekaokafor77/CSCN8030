...
def calculate_stochastic_oscillator(data, period=14):
    ...
    """
    percent_K = ta.stoch(data['High'], data['Low'], data['Close'], window=period)
    percent_D = ta.sma(percent_K, length=3) if percent_K is not None else None
    
    if percent_K is not None and percent_D is not None:
        stochastic_df = pd.DataFrame({'percent_K': percent_K.squeeze(), 'percent_D': percent_D.squeeze()}, index=data.index)
        return pd.concat([data, stochastic_df], axis=1)
    else:
        return data  # Return the original DataFrame if percent_K or percent_D is None
