...
def isPivot(candle, window, df):
    ...
    """
    # Assuming candle is a datetime objectcandle_timestamp = pd.Timestamp(candle)
    if candle - timedelta(days=window) < 0 or candle + timedelta(days=window) >= len(df):
        return 0

    pivotHigh = 1
    pivotLow = 2
    for i in range(candle - timedelta(days=window), candle + timedelta(days=window) + 1):
