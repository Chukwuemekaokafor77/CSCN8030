...
def display_advanced_analysis(selected_stock, start_date, end_date):
    ...
    df['MACD_Buy_Signal'] = np.where(df['MACD'] > df['Signal Line'], 1, 0)
    df['MACD_Sell_Signal'] = np.where(df['MACD'] < df['Signal Line'], 1, 0)

    fig, ax = plt.subplots()
    ax.plot(df.index, df['MACD'], label='MACD', color='blue')
    ax.plot(df.index, df['Signal Line'], label='Signal Line', color='red')
    ax.scatter(df.index, df[df['MACD_Buy_Signal'] == 1]['MACD'], marker='^', color='g', label='MACD Buy Signal')
    ax.scatter(df.index, df[df['MACD_Sell_Signal'] == 1]['MACD'], marker='v', color='r', label='MACD Sell Signal')
    ...
    df['RSI_Buy_Signal'] = np.where(df['RSI'] < 30, 1, 0)
    df['RSI_Sell_Signal'] = np.where(df['RSI'] > 70, 1, 0)
    ...
    ax.scatter(df.index, df[df['RSI_Buy_Signal'] == 1]['RSI'], marker='^', color='g', label='RSI Buy Signal')
    ax.scatter(df.index, df[df['RSI_Sell_Signal'] == 1]['RSI'], marker='v', color='r', label='RSI Sell Signal')
