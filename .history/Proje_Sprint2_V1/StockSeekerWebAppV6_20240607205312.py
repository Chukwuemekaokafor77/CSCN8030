...
def isPivot(candle, window, df):
    ...
    if candle_timestamp - datetime.timedelta(days=window) < 0 or candle_timestamp + datetime.timedelta(days=window) >= len(df):
        return 0

    pivotHigh = 1
    pivotLow = 2
    for i in range(candle_timestamp - timedelta(days=window), candle_timestamp + timedelta(days=window) + 1):
        if df.iloc[candle_timestamp].low > df.iloc[i].low:
            pivotLow = 0
        if df.iloc[candle_timestamp].high < df.iloc[i].high:
            pivotHigh = 0
